...
async function analyzeFile(
    ...
    if (mode === 'deep') {
        try {
            ...
            const { summary, issues: rawIssues, imports } = computeMetrics(ast, code, filePath);
...
export async function POST(req: NextRequest) {
    try {
        ...
        const packageJsonEntry = zip.getEntries().find(e => e.entryName.toLowerCase().endsWith('package.json'));
        if (packageJsonEntry) {
            try {
                ...
                projectDependencies = Array.from(new Set([...deps, ...devDeps]));
            } catch (err) {
                console.warn('[analyze-zip] Failed to parse package.json dependencies:', err);
            }
        }
        ...
        const poolResults = await pool(6, entries, async (entry) => {
            ...
            const result = await analyzeFile(filename, code, mode, entry.entryName);
        ...
        const tempAgg = aggregateResults(fileResults, '', allIssues, projectDependencies);
        ...
        const project = aggregateResults(fileResults, aiResponse.aiExplanation, allIssues, projectDependencies);
